...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		...
		synchronized (this.startupShutdownMonitor) {
			...
			// 1.准备刷新容器,记录下容器的启动时间、标记“已启动”状态、处理配置文件中的占位符
			// 1.1记录启动时间
			// 1.2设置原子属性(将 active 属性设置为 true，closed 属性设置为 false，它们都是 AtomicBoolean 类型)
			// 1.3填充property sources校验property
			prepareRefresh();
			...
			try {
				...
				// 4.【这里需要知道 BeanFactoryPostProcessor 这个知识点，Bean 如果实现了此接口，
				...
				// 7.初始化当前 ApplicationContext 的 MessageSource，国际化这里就不展开说了，不然没完没了了
				initMessageSource();

				// Initialize event multicaster for this context.
				// 8.初始化当前 ApplicationContext 的事件广播器，这里也不展开了
				...
				// 12.最后，广播事件，ApplicationContext 初始化完成
				finishRefresh();
			}
				...
				//13.销毁已经初始化完成的singletons的beans避免占用资源
				destroyBeans();
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 1.首先，初始化名字为 conversionService 的 Bean。本着送佛送到西的精神，我在附录中简单介绍了一下 ConversionService，因为这实在太实用了
		...
		// 2.先初始化 LoadTimeWeaverAware 类型的 Bean
		...
		// 3.没什么别的目的，因为到这一步的时候，Spring 已经开始预初始化 singleton beans 了，
